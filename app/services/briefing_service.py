...
def generate_prd_handoff_briefing(
    ...
    briefing = {
        ...
        "user": user_id,
        "memory_scope": f"agent:{from_agent_id}",
        "active_task": {
            "id": active_task["id"],
...
def list_briefings(
    ...
    with get_db() as conn:
        rows = conn.execute(
            f"""
            SELECT id, agent_id, user_id, assigned_agent_id, task_description,
                   status, memory_scope, started_at, updated_at, ended_at, metadata_json
            FROM agent_activity
            WHERE {where}
    ...
    for row in rows:
        ...
        briefing.setdefault("id", row["id"])
        briefing.setdefault("agent_id", row["agent_id"])
        briefing.setdefault("assigned_agent_id", row["assigned_agent_id"])
        briefing.setdefault("memory_scope", row["memory_scope"])
        briefing.setdefault("generated_at", row["updated_at"] or row["started_at"])
        briefing.setdefault("source_activity_id", briefing.get("source_activity_id"))
