...
def build_delegated_authority(context, header_value: str) -> EffectiveAuthority:
    ...
    with get_db() as conn:
        ...
        for permission in actions:
            ...
            if not binding or not binding.get("enabled") or not connector_type or not connector_type.get("is_active") or permission["action"] not in available or not issuer_authority.can_binding_action(permission["binding_id"], permission["action"], scope=binding["scope"]):
