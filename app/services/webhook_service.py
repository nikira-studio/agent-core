...
def test_delivery(webhook_id: str) -> dict:
    ...
    payload = {
        "event_type": "test",
        "timestamp": timestamp,
        "data": {"message": "Agent Core webhook test delivery", "webhook_id": webhook_id},
    }
    payload_json = json.dumps(payload, separators=(",", ":"))
    payload_bytes = payload_json.encode()
    ...
    try:
        response = httpx.post(
            ...
            headers={
                "Content-Type": "application/json",
                "X-Agent-Core-Signature": signature,
                "X-Agent-Core-Event": "test",
            },
            timeout=DELIVERY_TIMEOUT_SECONDS,
        )
        _record_delivery(webhook_id, "test", payload_json, "success" if 200 <= response.status_code < 300 else "failure", response.status_code, None if 200 <= response.status_code < 300 else f"HTTP {response.status_code}")
        return {"ok": True, "http_status": response.status_code}
    except httpx.TimeoutException:
        _record_delivery(webhook_id, "test", payload_json, "failure", None, "Delivery timed out")
        return {"ok": False, "error": "Delivery timed out"}
    except Exception as exc:
        msg = str(exc)[:200]
        _record_delivery(webhook_id, "test", payload_json, "failure", None, msg)
