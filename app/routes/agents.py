...
def _can_view_agent(agent: dict, session: dict) -> bool:
    if _can_manage_agent(agent, session):
        return True
    return agent_service.is_agent_shared(agent)
...
async def list_agents(session: dict = Depends(get_current_session)):
    if _is_admin(session):
        ...
        agents = agent_service.list_visible_agents(session["user_id"])
    safe_agents = []
    for agent in agents:
        safe_agents.append({
            "id": agent["id"],
...
async def get_agent(agent_id: str, session: dict = Depends(get_current_session)):
    ...
    if not _can_view_agent(agent, session):
