...
def test_create_agent(test_client, admin_token):
    ...
    read_scopes = json.loads(data["agent"]["read_scopes_json"])
    write_scopes = json.loads(data["agent"]["write_scopes_json"])
    assert "user:admin" in read_scopes
    assert "user:admin" not in write_scopes
...
def test_non_admin_cannot_grant_unowned_or_shared_write_scopes(
    ...
    unowned_project = test_client.post(
        ...
        json={
            "id": "badworkspace",
            "display_name": "Bad Workspace",
            "read_scopes": ["workspace:otherworkspace"],
        },
    )
...
def test_shared_agent_is_visible_but_read_only_to_other_users(
    ...
    create_user("viewer", "viewer@test.local", "testpassword123", "Viewer", "user")
    viewer_token = create_session("viewer")["session_id"]
    ...
    created = test_client.post(
        ...
        json={
            "id": "sharedviewagent",
            "display_name": "Shared View Agent",
        },
    )
    assert created.status_code == 201, created.json()

    fetched = test_client.get(
        "/api/agents/sharedviewagent",
        headers={"Authorization": f"Bearer {viewer_token}"},
    )
    assert fetched.status_code == 200, fetched.json()
    assert fetched.json()["data"]["agent"]["id"] == "sharedviewagent"
    assert fetched.json()["data"]["agent"]["is_shared"] is True
...
def test_agent_update_preserves_own_scope(test_client, admin_token):
    ...
    assert audit_service.query_events(action="agent_updated", resource_type="agent")[-1]["resource_id"] == "updatescopeagent"
...
def test_agent_purge_removes_scoped_data_and_access_references(
    test_client, admin_token
):
    import json
    from app.database import get_db
    ...
    activity_service.create_activity(
        "purgeagent", "admin", "Purge task", "agent:purgeagent"
    )
    ...
    with get_db() as conn:
        assert (
            conn.execute("SELECT 1 FROM agents WHERE id = 'purgeagent'").fetchone()
            is None
        )
        assert (
            conn.execute(
                "SELECT 1 FROM memory_records WHERE scope = 'agent:purgeagent'"
            ).fetchone()
            is None
        )
        assert (
            conn.execute(
                "SELECT 1 FROM credentials WHERE scope = 'agent:purgeagent'"
            ).fetchone()
            is None
        )
        assert (
            conn.execute(
                "SELECT 1 FROM agent_activity WHERE agent_id = 'purgeagent'"
            ).fetchone()
            is None
        )
        reader = conn.execute(
