...
def test_embedding_write_with_mocked_healthy_backend(test_client, agent_token):
    ...
    with (
        ...
        with patch("app.services.vector_service.store_embedding") as mock_store:
            ...
            _record_id = write_r.json()["data"]["record"]["id"]
...
def test_search_hybrid_pagination_applies_after_merge(test_client, agent_token):
    ...
    with (
        ...
        for _ in range(3):
            write_r = test_client.post(
                ...
                headers={"Authorization": f"Bearer {agent_token}"},
                json={
                    "content": "Pagination hybrid token",
                    "memory_class": "fact",
                    "scope": "agent:testagent",
                },
            )
            assert write_r.status_code == 201, write_r.json()
            record_ids.append(write_r.json()["data"]["record"]["id"])

    def fake_cosine_search(_vector_bytes, _top_k, candidate_ids):
    ...
    with (
        ...
        patch("app.services.vector_service.cosine_search_top_k", side_effect=fake_cosine_search),
    ):
        mock_status.return_value = {"backend": "healthy", "model_configured": True}
        mock_gen.return_value = (mock_vector_bytes, "ok")
        mock_vec.return_value = True
        ...
        search_r = test_client.post(
            "/api/memory/search",
            headers={"Authorization": f"Bearer {agent_token}"},
            json={"query": "Pagination hybrid token", "limit": 1, "offset": 1},
        )

    assert search_r.status_code == 200, search_r.json()
    data = search_r.json()["data"]
    assert data["retrieval_mode"] == "hybrid"
    assert [r["id"] for r in data["records"]] == [record_ids[1]]
...
def test_mcp_memory_search_returns_retrieval_mode_and_backend_status(
...
def test_mcp_tool_memory_search_empty_scopes_returns_embedding_status(
