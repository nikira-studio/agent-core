...
def test_credential_entry_response_no_raw_secret(test_client, agent_token):
    r = test_client.post(
...
def test_credential_list_no_raw_secrets(test_client, agent_token):
    test_client.post(
        "/api/credentials/entries",
        headers={"Authorization": f"Bearer {agent_token}"},
        json={
