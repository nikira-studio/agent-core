...
def test_project_create(test_client, admin_token):
    ...
    assert audit_service.query_events(action="workspace_created", resource_type="workspace")[-1]["resource_id"] == "test-workspace"
...
def test_non_admin_can_manage_owned_project(test_client, admin_token):
    ...
    assert audit_service.query_events(action="workspace_updated", resource_type="workspace")[-1]["resource_id"] == "owned-workspace"
...
def test_project_purge_removes_scoped_data_and_access_references(
    ...
    memory_service.write_memory(
        "workspace scoped memory", "fact", "workspace:purge-workspace"
    )
    ...
    with get_db() as conn:
        assert (
            conn.execute(
                "SELECT 1 FROM workspaces WHERE id = 'purge-workspace'"
            ).fetchone()
            is None
        )
        assert (
            conn.execute(
                "SELECT 1 FROM memory_records WHERE scope = 'workspace:purge-workspace'"
            ).fetchone()
            is None
        )
        assert (
            conn.execute(
                "SELECT 1 FROM credentials WHERE scope = 'workspace:purge-workspace'"
            ).fetchone()
            is None
        )
        agent = conn.execute(
